...
function global:au_GetLatest {
    ...
    $latest_release = Invoke-RestMethod -Method Get -Uri 'https://api.github.com/repos/mongodb-js/mongosh/releases/latest'
        
    foreach( $asset in $latest_release.assets) 
    {
        if ($asset.browser_download_url.Contains("win32-zip"))
        {
